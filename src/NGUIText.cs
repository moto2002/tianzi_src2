...
public static class NGUIText
{
	...
	public static Color ParseColor(string text, int offset)
	{
		...
		clear.r = num4 * (float)num;
		clear.g = num4 * (float)num2;
		clear.b = num4 * (float)num3;
		return clear;
	}
	...
	public static bool ParseSymbol(string text, ref int index, BetterList<Color> colors, bool premultiply, ref int sub, ref bool bold, ref bool italic, ref bool underline, ref bool strike)
	{
		...
		if (text[index + 2] == ']')
		{
			...
			if (string.Compare("[i]", 0, text, index, 3) == 0)
			{
				italic = true;
				index += 3;
				return true;
			}
			if (string.Compare("[u]", 0, text, index, 3) == 0)
			{
				underline = true;
				index += 3;
				return true;
			}
			if (string.Compare("[s]", 0, text, index, 3) == 0)
			{
				strike = true;
				index += 3;
				return true;
		...
		if (text[index + 3] == ']')
		{
			...
			if (string.Compare("[/i]", 0, text, index, 4) == 0)
			{
				italic = false;
				index += 4;
				return true;
			}
			if (string.Compare("[/u]", 0, text, index, 4) == 0)
			{
				underline = false;
				index += 4;
				return true;
			}
			if (string.Compare("[/s]", 0, text, index, 4) == 0)
			{
				strike = false;
				index += 4;
				return true;
		...
		if (text[index + 4] == ']')
		{
			...
			if (string.Compare("[sup]", 0, text, index, 5) == 0)
			{
				sub = 2;
				index += 5;
				return true;
		...
		if (text[index + 5] == ']')
		{
			...
			if (string.Compare("[/sup]", 0, text, index, 6) == 0)
			{
				sub = 0;
				index += 6;
				return true;
			}
			if (string.Compare("[/url]", 0, text, index, 6) == 0)
			{
				index += 6;
				return true;
			}
		...
		string strA = NGUIText.EncodeColor(color);
		if (string.Compare(strA, 0, text, index + 1, 6, true) != 0)
		{
			return false;
		}
		if (colors != null)
	...
	public static bool CustomWrapText(string text, out string finalText)
	{
		...
		while (i < length)
		{
			...
			if (c == '\n')
			{
				...
				if (num5 < i)
				{
					stringBuilder.Append(text, num5, i - num5 + 1);
				}
				else
				{
			...
			else
			{
				...
				if (bMSymbol == null)
				{
					...
					num7 = NGUIText.spacingX + glyphWidth;
				}
				else
				{
					num7 = NGUIText.spacingX + (float)bMSymbol.advance;
				}
				num4 -= num7;
				if (num4 < 0f)
				{
					if (flag || num6 == num3)
					{
						stringBuilder.Append(text, num5, Mathf.Max(0, i - num5));
						if (num6++ == num3)
						{
							num5 = i;
							break;
						...
						{
							num5 = i;
							num4 = (float)NGUIText.rectWidth - num7;
						}
						prev = 0;
					}
					else
		...
		if (num5 < i)
		{
			stringBuilder.Append(text, num5, i - num5);
		}
	...
	public static bool WrapText(string text, out string finalText, bool keepCharCount)
	{
		...
		while (i < length)
		{
			...
			if (c == '\n')
			{
				...
				if (num4 < i)
				{
					stringBuilder.Append(text, num4, i - num4 + 1);
				}
				else
				{
					stringBuilder.Append(c);
			...
			else
			{
				...
				if (c == ' ' && !flag3)
				{
					...
					else if (num6 != 32 && num4 < i)
					{
						...
						stringBuilder.Append(text, num4, num8);
						flag = false;
						num4 = i + 1;
					}
				}
				if (Mathf.RoundToInt(num3) < 0)
				{
					if (flag || num5 == num2)
					{
						...
						stringBuilder.Append(text, num4, Mathf.Max(0, i - num4));
						if (num5++ == num2)
						{
							num4 = i;
							break;
		...
		if (num4 < i)
		{
			stringBuilder.Append(text, num4, i - num4);
		}
		finalText = stringBuilder.ToString();
			...
			else
			{
				...
				else
				{
					...
					if (glyphInfo != null)
					{
						...
						if (num8 != 0)
						{
							NGUIText.GlyphInfo expr_4BC_cp_0 = glyphInfo;
							expr_4BC_cp_0.v0.x = expr_4BC_cp_0.v0.x * 0.75f;
							NGUIText.GlyphInfo expr_4D4_cp_0 = glyphInfo;
							expr_4D4_cp_0.v0.y = expr_4D4_cp_0.v0.y * 0.75f;
							NGUIText.GlyphInfo expr_4EC_cp_0 = glyphInfo;
							expr_4EC_cp_0.v1.x = expr_4EC_cp_0.v1.x * 0.75f;
							NGUIText.GlyphInfo expr_504_cp_0 = glyphInfo;
							expr_504_cp_0.v1.y = expr_504_cp_0.v1.y * 0.75f;
							if (num8 == 1)
							{
								NGUIText.GlyphInfo expr_524_cp_0 = glyphInfo;
								expr_524_cp_0.v0.y = expr_524_cp_0.v0.y - NGUIText.fontScale * (float)NGUIText.fontSize * 0.4f;
								NGUIText.GlyphInfo expr_549_cp_0 = glyphInfo;
								expr_549_cp_0.v1.y = expr_549_cp_0.v1.y - NGUIText.fontScale * (float)NGUIText.fontSize * 0.4f;
							}
							else
							{
								NGUIText.GlyphInfo expr_573_cp_0 = glyphInfo;
								expr_573_cp_0.v0.y = expr_573_cp_0.v0.y + NGUIText.fontScale * (float)NGUIText.fontSize * 0.05f;
								NGUIText.GlyphInfo expr_598_cp_0 = glyphInfo;
								expr_598_cp_0.v1.y = expr_598_cp_0.v1.y + NGUIText.fontScale * (float)NGUIText.fontSize * 0.05f;
							}
						}
							...
							if (uvs != null)
							{
								...
								while (l < num15)
								{
									...
									else
									{
										...
										zero4.x = glyphInfo.u0.x;
										zero4.y = glyphInfo.u1.y;
										uvs.Add(zero4);
										uvs.Add(glyphInfo.u1);
										zero4.x = glyphInfo.u1.x;
										zero4.y = glyphInfo.u0.y;
										uvs.Add(zero4);
									}
									l++;
								}
