	...
	public class USInternalCurve : ScriptableObject
	{
		...
		private void OnEnable()
		{
			...
			else if (this.internalKeyframes.Count > 0)
			{
				try
				{
					IOrderedEnumerable<USInternalKeyframe> orderedEnumerable = from keyframe in this.internalKeyframes
					orderby keyframe.Time
					select keyframe;
					if (orderedEnumerable != null)
					{
						USInternalKeyframe uSInternalKeyframe = orderedEnumerable.LastOrDefault<USInternalKeyframe>();
						if (uSInternalKeyframe != null)
						{
							this.Duration = uSInternalKeyframe.Time;
						}
					}
				}
				catch (Exception var_2_8A)
				{
				}
			}
