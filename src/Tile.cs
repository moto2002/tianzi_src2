...
public class Tile
{
	...
	public void Update(Vector3 eyePos)
	{
		...
		if (!GameScene.isPlaying && this.water != null)
		{
		...
		if (this.tick % 2 == 0)
		{
			...
			while (this.unitInd < this.unitCount)
			{
				...
				if (!gameObjectUnit.active && (gameObjectUnit.combineParentUnitID < 0 || GameScene.SampleMode))
				{
					...
					if (gameObjectUnit.viewDistance < gameObjectUnit.near)
					{
	...
	public void DrawGridData()
	{
		...
		for (int i = 0; i < num; i++)
		{
			for (int j = 0; j < num2; j++)
			{
				float num7 = (float)i;
				float num8 = (float)j;
				float num9 = num7 * gridSize - (float)terrainConfig.tileSize * 0.5f;
				float num10 = num8 * gridSize - (float)terrainConfig.tileSize * 0.5f;
				int gridX = this.getGridX(num9 + this.position.x);
				int gridY = this.getGridY(num10 + this.position.z);
				float y;
				if ((this.region.scene.mapPath.GetGridValue(gridX, gridY) & 13) <= 0)
				{
					y = -1000f;
				}
				else
	...
	{
		return Mathf.FloorToInt(value / this.region.scene.terrainConfig.gridSize) + this.region.scene.mapPath.miHalfOrignWidth;
	}

	public int getGridY(float value)
	{
		return Mathf.FloorToInt(value / this.region.scene.terrainConfig.gridSize) + this.region.scene.mapPath.miHalfOrignHeight;
	}
}
