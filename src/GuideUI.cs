...
public class GuideUI : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		this.uiCamera = gameObject.camera;
	...
	public GameObject ShowMBox(int iType, string strInfo, EventDelegate.Callback onMok, EventDelegate.Callback onMCancel, string strTitle = "")
	{
		...
		if (this.moMessageBox == null)
		{
			...
			this.moMessageBox = (Resources.Load(path) as GameObject);
		}
		if (this.moMessageBox == null)
		{
			return null;
	...
	public void SetVersion(string strRemoteVersion, string strDestVersion, string strRemoteLangVersion, string strDestLangVersion)
	{
		...
		if (this.mlblSourceVersion != null)
		{
			...
			{
				this.mlblSourceVersion.text = "0.0.0 " + strDestLangVersion;
			}
			else
			{
				this.mlblSourceVersion.text = strDestVersion + " " + strDestLangVersion;
			}
		}
		if (this.mlblDestVersion != null)
		{
			this.mlblDestVersion.text = strRemoteVersion + " " + strRemoteLangVersion;
		}
	}
	...
	public void CreateBackPanel()
	{
		...
		{
			string path = string.Format("Local/Prefabs/UI/{0}/BackPanel", Language.LanguageVersion.ToString());
			...
			if (this.muiBackPanel != null)
			{
				...
				string text = string.Format(this.GetConfig("GuideUI", "EffectName"), Language.LanguageVersion.ToString());
				if (!string.IsNullOrEmpty(text))
				{
					...
					if (gameObject != null)
					{
						...
						if (this.muiEffect != null)
						{
							...
							if (transform != null)
							{
								...
								Vector3 zero = Vector3.zero;
								float configToFloat = this.GetConfigToFloat("GuideUI", "PosX");
								float configToFloat2 = this.GetConfigToFloat("GuideUI", "PosY");
								float configToFloat3 = this.GetConfigToFloat("GuideUI", "PosZ");
								zero.x = configToFloat;
								zero.y = configToFloat2;
								zero.z = configToFloat3;
								this.muiEffect.transform.localPosition = zero;
								float configToFloat4 = this.GetConfigToFloat("GuideUI", "RotX");
								float configToFloat5 = this.GetConfigToFloat("GuideUI", "RotY");
								float configToFloat6 = this.GetConfigToFloat("GuideUI", "RotZ");
								zero.x = configToFloat4;
								zero.y = configToFloat5;
								zero.z = configToFloat6;
								this.muiEffect.transform.localRotation = Quaternion.Euler(zero);
								float configToFloat7 = this.GetConfigToFloat("GuideUI", "Scale");
								this.muiEffect.transform.localScale = Vector3.one * configToFloat7;
	...
	public void ShowLoadingPanel()
	{
		if (this.uiLoading == null)
		{
			string path = string.Format("Local/Prefabs/UI/{0}/LoadingPanel", Language.LanguageVersion.ToString());
			GameObject original = Resources.Load(path) as GameObject;
