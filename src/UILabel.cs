...
public class UILabel : UIWidget
{
	...
	[HideInInspector, SerializeField]
	private string mstrFontName = string.Empty;
	...
	public string FontName
	{
		...
		set
		{
			if (this.mstrFontName != value)
			{
				this.mstrFontName = value;
				AtlasManager.GetFont(this.mstrFontName, base.gameObject, new AssetCallBack(this.OnLoadComplete));
			}
		}
	}
	...
	public void ChangeFont()
	{
		...
		if (!string.IsNullOrEmpty(this.mstrFontName))
		{
			if (this.bitmapFont != null)
			{
				AtlasManager.PopFont(this.mstrFontName, base.gameObject);
			}
			AtlasManager.GetFont(this.mstrFontName, base.gameObject, new AssetCallBack(this.OnLoadComplete));
		}
	}
	...
	protected override void Awake()
	{
		if (!string.IsNullOrEmpty(this.mstrFontName))
		{
			...
			if (text.Equals("SnailFont"))
			{
				text = "NGUIFont";
			}
			else if (text.Equals("TextFont"))
			{
				text = "NGUIFont1";
			}
			AtlasManager.GetFont(text, base.gameObject, new AssetCallBack(this.OnLoadComplete));
		}
	}

	private void OnLoadComplete(VarStore args)
	{
		UIFont uIFont = args[0] as UIFont;
		if (uIFont != null)
		{
			this.bitmapFont = uIFont;
			this.MarkAsChanged();
	...
	private void OnDestroy()
	{
		if (this.bitmapFont != null)
		{
			AtlasManager.PopFont(this.FontName, base.gameObject);
		}
	}
	...
	public override void CheckLoadAtlas()
	{
		...
		if (this.bitmapFont == null && !string.IsNullOrEmpty(this.FontName))
		{
			AtlasManager.GetFont(this.FontName, base.gameObject, new AssetCallBack(this.OnLoadComplete));
		}
	}

	public override bool CheckWaitLoadingAtlas()
	{
		return !this.error && base.gameObject.activeInHierarchy && !string.IsNullOrEmpty(this.FontName) && this.mFont == null;
	}
