...
namespace WellFired
{
	...
	public class UpdateSceneSphereAction : ActionBase
	{
		...
		public override void ProcessEvent(float runningTime)
		{
			try
			{
				if (!SequenceManager.GetInstance().isEditor && this.sphere != null && SequenceManager.GetInstance().UpdateScene != null)
				{
					SequenceManager.GetInstance().UpdateScene(this.sphere.transform.position);
				}
			}
			catch (Exception ex)
			{
