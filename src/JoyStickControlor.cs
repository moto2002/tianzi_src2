...
public class JoyStickControlor : MonoBehaviour
{
	...
	private void CreateMouseObj()
	{
		...
		string.Format("Prefabs/UI/{0}/MousePanel", Language.LanguageVersion);
		DelegateProxy.LoadAsset(empty, new AssetCallBack(this.OnPrefabLoaded), false);
	}

	private void OnPrefabLoaded(VarStore param)
	{
		...
		DelegateProxy.UnloadAsset(new object[]
		{
			param
		});
	}
