...
public class FileUtils
{
	...
	public static string GetDownloadUrl(string fileurl)
	{
		...
		{
			fileurl = fileurl + "?p=" + FileUtils.GetHttpReqPlus();
		}
		return fileurl;
	}
	...
	public static string GetStringMd5Utf8(string strValue)
	{
		byte[] bytes = Encoding.UTF8.GetBytes(strValue);
		return FileUtils.GetByteMd5BIT64(bytes);
	}

	public static string GetStringMd5(string strValue)
	{
		byte[] bytes = Encoding.Default.GetBytes(strValue);
		return FileUtils.GetByteMd5(bytes);
	}

	public static string GetByteMd5BIT64(byte[] bytes)
	{
		MD5 mD = MD5.Create();
