...
public class DynamicUnit : GameObjectUnit
{
	...
	public bool bLookAtTarget = true;
	...
	public override void Update()
	{
		if (this.ins != null)
		{
			...
			if (Mathf.Abs(this.insTrans.rotation.x - this.rotation.x) > 0.01f || Mathf.Abs(this.insTrans.rotation.y - this.rotation.y) > 0.01f || Mathf.Abs(this.insTrans.rotation.z - this.rotation.z) > 0.01f)
			{
		...
		if (this.mDynState == DynamicState.LINK_PARENT || this.mDynState == DynamicState.LINK_PARENT_CHILD)
		{
			for (int j = 0; j < this.linkUnits.Count; j++)
			{
				...
				{
					this.linkUnits[j].Update(this.position, this.rotation, this.moving);
				}
			}
		}
	...
	public void OnPathFindEnd(List<Vector3> paths)
	{
		...
		this.pathInterrupted = false;
		this.Move(paths[0], this.speed, true);
		if (paths.Count > 1)
		{
			paths.RemoveAt(0);
		}
	...
	public void Move(Vector3 target, float speed, bool bLookAtTarget = true)
	{
		if (this.destroyed)
		{
			return;
		}
		this.bLookAtTarget = bLookAtTarget;
		this.move_type = 0;
		this._startMove = 1;
		this._endMove = 0;
		this.moving = true;
	...
	public bool TryMove(Vector3 target)
	{
		...
		this.nextTarget = this.from + this.tryStep;
		while (this.scene.IsValidForWalk(this.nextTarget, this.collisionSize, out this.gridType) || (this.gridType & 4) > 0)
		{
			float num2 = Vector3.Distance(this.from, this.nextTarget);
			if (num2 >= num)
			{
