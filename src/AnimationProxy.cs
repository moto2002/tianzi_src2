...
public class AnimationProxy : MonoBehaviour
{
	...
	public delegate void OnMainClipLoaded(AnimationProxy animationProxy);

	public delegate void CallLoadAsset(string strFileName, AssetCallBack callback, VarStore store = null);

	private const int miActionListMaxCount = 10;
	...
	private AnimationProxy.OnMainClipLoaded onMainClipLoaded;
	...
	private void Awake()
	{
		...
		if (!string.IsNullOrEmpty(this.modelAcionID))
		{
			...
			DelegateProxy.LoadAsset(strFileName, new AssetCallBack(this.AniInfoAssetLoaded), false);
		}
		else
		{
			this.OnStart();
	...
	private void AniInfoAssetLoaded(VarStore args)
	{
	...
	public void SetOnMainClipLoaded(AnimationProxy.OnMainClipLoaded call, string activeAction)
	{
		this.onMainClipLoaded = call;
		this.activeAction = activeAction;
		if (!string.IsNullOrEmpty(this.activeAction))
		{
			if (this.mActionLoadedDict.ContainsKey(this.activeAction) && this.onMainClipLoaded != null)
			{
				this.onMainClipLoaded(this);
			}
		}
		else if (this.mMainClip != null && !string.IsNullOrEmpty(this.mMainClip.strName) && this.mActionLoadedDict.ContainsKey(this.mMainClip.strName) && this.onMainClipLoaded != null)
		{
			this.onMainClipLoaded(this);
		}
	}
	...
	public void LoadAnimationClip(string strAnimationName)
	{
		...
		{
			AnimationProxy.monLoadAsset(animationPath, delegate(VarStore args)
			{
	...
	private void LoadAnimationClipNoPlay(string strAnimationName)
	{
		string animationPath = this.GetAnimationPath(strAnimationName);
		if (string.IsNullOrEmpty(animationPath))
		{
	...
	private void PreLoadAction(string strAction)
	{
		...
		if (this.mActionControlList.Count >= 10)
		{
			return;
		}
	...
	private void CheckAnimationPlay()
	{
	...
	private bool IsAnimationClipLoaded(string strAnimation)
	{
	...
	private void AddAnimationClip(string strClipName, AnimationClip aClip)
	{
		...
		if (!string.IsNullOrEmpty(this.activeAction))
		{
			if (strClipName == this.activeAction && this.onMainClipLoaded != null)
			{
				this.onMainClipLoaded(this);
			}
		}
		else if (this.mMainClip != null && !string.IsNullOrEmpty(this.mMainClip.strName) && this.mMainClip.strName == strClipName && this.onMainClipLoaded != null)
		{
			this.onMainClipLoaded(this);
		}
	...
	private void SequenceList(string strAnimation)
	{
	...
	public void RefreshAniComponent()
	{
		if (this.mAnimation != null)
		{
			this.mAnimation.enabled = false;
