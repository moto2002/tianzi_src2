...
public class UIGrid : UIWidgetContainer
{
	...
	public delegate void OnDoubleClickSubItem(UIGridItem oItem, GameObject OSubItem);

	public delegate void OnPressSubItem(UIGridItem oItem, GameObject OSubItem, bool isdown);
	...
	public UIGrid.OnDoubleClickSubItem fnonDoubleClickSubItem;

	public UIGrid.OnPressSubItem fnonPressSubItem;
	...
	public void OnDoubleClickItem(UIGridItem oItem, GameObject OSubItem)
	{
		if (this.fnonDoubleClickSubItem != null)
		{
			this.fnonDoubleClickSubItem(oItem, OSubItem);
		}
	}

	public void OnPressItem(UIGridItem oItem, GameObject OSubItem, bool isdown)
	{
		if (this.fnonPressSubItem != null)
		{
			this.fnonPressSubItem(oItem, OSubItem, isdown);
		}
	}
	...
	public void DeleteCustomData(object oData, bool bUpdateCustomView)
	{
		if (oData == null)
		{
			return;
	...
	public void UpdateCustomDataListEx(List<object> list)
	{
		int count = list.Count;
		int count2 = this.mDataSource.Count;
		int num = Mathf.Max(count, count2);
		...
		this.UpdateCustomViewEx();
	}

	public void UpdateCustomDataListExForStrengthen(List<object> list)
	{
		int count = list.Count;
		int count2 = this.mDataSource.Count;
		int num = Mathf.Max(count, count2);
		...
		this.UpdateCustomViewExForStrengthen();
	}
	...
	public List<object> GetActiveItems()
	{
		...
		if (this.arrangement == UIGrid.Arrangement.Vertical)
		{
			num = (int)(this.mPanel.height / this.cellHeight) + 2;
		}
		else if (this.arrangement == UIGrid.Arrangement.Horizontal)
		{
			num = Mathf.FloorToInt(this.mPanel.width / this.cellWidth) + 2;
		}
		int num2 = this.mOffsetRows * this.maxPerLine;
		int num3 = num2 + num * this.maxPerLine;
		List<object> list = new List<object>();
		for (int i = 0; i < this.mDataSource.Count; i++)
		{
			object obj = this.mDataSource[i];
			Transform transform;
			if (this.mDataToTrans.TryGetValue(obj, out transform))
			{
				UIGridItem component = transform.GetComponent<UIGridItem>();
				if (component.miCurIndex >= num2 && component.miCurIndex <= num3 && this.mPanel.IsVisible((UIWidget)component.mScripts[0]))
				{
					list.Add(obj);
				}
			}
	...
	public List<object> GetActiveData()
	{
		List<object> list = new List<object>();
		int num = 0;
		if (this.arrangement == UIGrid.Arrangement.Vertical)
		...
		int num2 = this.mOffsetRows * this.maxPerLine;
		int num3 = num2 + num * this.maxPerLine;
		foreach (KeyValuePair<object, Transform> current in this.mDataToTrans)
		{
			UIGridItem component = current.Value.GetComponent<UIGridItem>();
			if (component.miCurIndex >= num2 && component.miCurIndex <= num3 && this.mPanel.IsVisible((UIWidget)component.mScripts[0]))
			{
	...
	private void UpdateCustomViewExForStrengthen()
	{
		int num = 0;
		int num2 = 0;
		int num3 = 0;
		...
		int num4 = Mathf.CeilToInt((float)this.mDataSource.Count / (float)this.maxPerLine);
		int num5 = num4 - num3;
		bool flag = false;
		if (this.mOffsetRows > num5)
		{
			this.mOffsetRows = ((num5 <= 0) ? 0 : num5);
			flag = true;
		}
		int num6 = this.mOffsetRows * this.maxPerLine;
		int num7 = num6 + num3 * this.maxPerLine;
		for (int i = 0; i < this.mDataSource.Count; i++)
		{
			object obj = this.mDataSource[i];
			if (i < num6 || i > num7)
		...
		if (flag)
		{
			this.GoToPosition(num6);
		}
	}

	private void UpdateCustomViewEx()
	{
		int num = 0;
		int num2 = 0;
		int num3 = 0;
	...
	private void OnDestroy()
	{
		...
		this.fnonDoubleClickSubItem = null;
		this.fnonPressSubItem = null;
