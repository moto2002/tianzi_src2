...
public class GameObjectUnit
{
	...
	public delegate void ThridPardResourManager(string strFileName, AssetCallBack back, VarStore store = null);
	...
	public void RotaionY(float deg, bool immediately = false)
	{
		...
		{
			this.insTrans.rotation = this.rotation;
		}
	}
	...
	public virtual void Update()
	{
		...
		{
			if (Mathf.Abs(this.insTrans.position.x - this.position.x) > 0.01f || Mathf.Abs(this.insTrans.position.z - this.position.z) > 0.01f)
			{
				this.ComputeTiles();
			}
			this.position = this.insTrans.position;
			this.rotation = this.insTrans.rotation;
			this.localScale = this.insTrans.localScale;
			this.unitParser.Update(this.ins);
		}
		this.tick++;
	}

	private void OnThridPartAssetLoaded(VarStore args)
	{
	...
	private void Initialize()
	{
		...
		if (this.ins == null && this.pre != null)
		{
			...
			this.insTrans = this.ins.transform;
			...
			this.insTrans.position = this.position;
			...
			{
				this.insTrans.rotation = this.rotation;
				this.insTrans.localScale = this.localScale;
				if (LightmapSettings.lightmaps.Length > 0)
				{
					...
					for (int j = 0; j < count; j++)
					{
						...
						else if (lightmapPrototype.rendererChildIndex < this.insTrans.childCount)
						{
							renderer = this.insTrans.GetChild(lightmapPrototype.rendererChildIndex).renderer;
						}
						if (renderer != null)
						{
				...
				for (int k = 0; k < this.insTrans.childCount; k++)
				{
					Renderer renderer2 = this.insTrans.GetChild(k).renderer;
					if (renderer2 != null)
					{
						list.Add(renderer2);
					}
		...
		else
		{
			...
			if (GameScene.isEditor)
			{
				...
				this.insTrans.position = this.position;
				this.insTrans.rotation = this.rotation;
				this.insTrans.localScale = new Vector3(2f, 2f, 2f);
	...
	private void PlayBornEffect()
	{
		...
		{
			DelegateProxy.LoadAsset(this._bornEffectPrePath, delegate(VarStore args)
			{
				GameObject gameObject = args[0] as GameObject;
				if (gameObject != null)
				{
					this.bornEffect = (DelegateProxy.Instantiate(gameObject, Vector3.zero, Quaternion.identity) as GameObject);
			...
			if (this.ins == null)
			{
				if (this.prePath != string.Empty)
				{
					if (GameObjectUnit.thridPardResourManager != null && this.scene.loadFromAssetBund)
					{
						...
						{
							GameObjectUnit.thridPardResourManager(this.prePath, new AssetCallBack(this.OnThridPartAssetLoaded), null);
						}
					}
					else
					{
	...
	public void Renamme()
	{
		if (!GameScene.isPlaying)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (lightmapPrototype.rendererChildIndex > -1)
				{
					if (lightmapPrototype.rendererChildIndex < this.insTrans.childCount)
					{
						renderer = this.insTrans.GetChild(lightmapPrototype.rendererChildIndex).renderer;
					}
					if (renderer != null)
	...
	private void AddMeshRenderColliders()
	{
		if (this.ins != null)
		{
			if (this.insTrans.renderer && this.ins.GetComponent<MeshCollider>() == null)
			{
				this.ins.AddComponent<MeshCollider>();
			}
			int childCount = this.insTrans.childCount;
			for (int i = 0; i < childCount; i++)
			{
				Renderer renderer = this.insTrans.GetChild(i).renderer;
	...
	public BoxCollider ComputeBounds()
	{
		...
		Vector3 min = this.insTrans.InverseTransformPoint(bounds.min);
		Vector3 max = this.insTrans.InverseTransformPoint(bounds.max);
		bounds.min = min;
		bounds.max = max;
		BoxCollider boxCollider = this.ins.AddComponent<BoxCollider>();
		boxCollider.center = bounds.center;
		boxCollider.size = bounds.size;
		this.center = this.insTrans.localToWorldMatrix.MultiplyPoint3x4(bounds.center);
		return boxCollider;
	}
}
